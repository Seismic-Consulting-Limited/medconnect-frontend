...
export default function HospitalDetailPage({ params }: { params: HospitalParams }) {
  // Get the ID directly from params without using React.use()
  // This is simpler and avoids TypeScript errors
  const id = params.id

  const [showGate, setShowGate] = useState(false)
  const [gateType, setGateType] = useState<"hospital" | "treatment" | "pricing" | "contact">("hospital")
  const { trackView, loaded, debug } = useViewTracker()

  // Add debugging on mount
  useEffect(() => {
    console.log("[HospitalDetailPage] Mounted with ID:", id)
    // Call debug function to log current state
  ...
  useEffect(() => {
    if (loaded) {
      console.log("[HospitalDetailPage] ViewTracker loaded, tracking view for hospital:", id)
      try {
        const shouldPrompt = trackView("hospital", id)
        console.log("[HospitalDetailPage] Should prompt:", shouldPrompt)
        if (shouldPrompt) {
          setGateType("hospital")
          setShowGate(true)
        }
      } catch (error) {
        console.error("[HospitalDetailPage] Error tracking view:", error)
      }
    } else {
      console.log("[HospitalDetailPage] ViewTracker not loaded yet")
    }
  }, [id, loaded, trackView])
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1">
          ...
          <ResponsiveContainer>
            ...
            <Tabs defaultValue="treatments" className="w-full">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                          ...
                          <div className="flex flex-col items-end gap-2">
                            {index === 0 ? (
                              ...
                              <PremiumContentOverlay type="pricing" height="h-24" />
                            )}
                          </div>
                        </div>
                      </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {doctors.map((doctor, index) => (
                      <Card key={doctor.id} className={index >= 1 ? "hidden md:block" : ""}>
                        <CardContent className="p-6">
                          <div className="flex flex-col items-center text-center">
                            ...
                            {index === 0 ? (
                              ...
                              <PremiumContentOverlay type="details" height="h-10" />
                            )}
                          </div>
                        </CardContent>
                      </Card>
              ...
              <TabsContent value="facilities" className="mt-2">
                <PremiumContentOverlay type="details" height="h-80" />
              </TabsContent>
        ...
        <section className="w-full py-8 md:py-12 lg:py-16 bg-white">
          <ResponsiveContainer>
            <div className="grid gap-8 lg:grid-cols-2 items-center">
              ...
              <PremiumContentOverlay type="contact" height="h-96" />
            </div>
          </ResponsiveContainer>
        </section>
        ...
        {showGate && <SoftGate type={gateType} onClose={() => setShowGate(false)} />}
      </main>
      <SiteFooter />
    </div>
  )

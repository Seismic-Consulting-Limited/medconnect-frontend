...
export default function ClientSignupPage() {
  ...
  // Client-side password checks (align with backend rules as needed)
  const validatePassword = (pwd: string) => {
    const hasUppercase = /[A-Z]/.test(pwd)
    const hasLowercase = /[a-z]/.test(pwd)
    const hasMinLength = pwd.length >= 6
    const hasNumber = /\d/.test(pwd) // include if your backend requires a digit
              ...
              {password && (
                <div className="text-sm space-y-1">
                  <p className="text-primary">
                    Passwords must be case-sensitive, include{" "}
                    <span className={passwordValidation.hasUppercase ? "text-primary" : "text-destructive"}>uppercase</span>{" "}
                    and{" "}
                    <span className={passwordValidation.hasLowercase ? "text-primary" : "text-destructive"}>
                      lowercase
                    </span>{" "}
                    letters, contain{" "}
                    <span className={passwordValidation.hasNumber ? "text-primary" : "text-destructive"}>a number</span>, and
                    be at{" "}
                    <span className={passwordValidation.hasMinLength ? "text-primary" : "text-destructive"}>

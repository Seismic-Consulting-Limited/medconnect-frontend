  ...
  public async getHospitalProfileCompletionRate(): Promise<{ completion_rate: number }> {
    try {
      ...
      const response = await apiRequest<{ data: { completion_rate: number } }>(
        API_ENDPOINTS.META.HOSPITAL_PROFILE_COMPLETION_RATE,
        { method: HTTP_METHODS.GET },
        { auth: true, getToken: () => authService.getToken() },
      );
      ...
      console.error("Failed to fetch hospital profile completion rate:", error);
    ...
    try {
      ...
      return {
        ...
        location: {
          ...
          state: { id: 1, name: "Sample State", code: "SS" },
        },
      };
    }
  }

  public async getTravelAgentProfileCompletionRate(): Promise<{ completion_rate: number }> {
    try {
      await this.validateAuthOrThrow();

      const response = await apiRequest<{ data: { completion_rate: number } }>(
        API_ENDPOINTS.META.TRAVEL_AGENT_PROFILE_COMPLETION_RATE,
        { method: HTTP_METHODS.GET },
        { auth: true, getToken: () => authService.getToken() },
      );
      ...
      console.error("Failed to fetch travel agent profile completion rate:", error);
      return { completion_rate: 0 };
    }
  }
}
